...
export default function editarmarca() {
  ...
  const validation = () => {
    ...
    if (nombre === '') {
      messajeTool('error', 'Debe asignar un nombre a la marca.');
      setError(true);
      return false;
    }
  ...
  const actualizar = async () => {
    ...
    try {
      ...
      const { data } = await axios.put(`${URLAPIGENERAL}/marcas`, Jsonenvio, {
        headers: {
  ...
  React.useEffect(() => {
    async function obtenerMarca() {
      try {
        const res = await axios(`${URLAPIGENERAL}/marcas/buscar?codigo=${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }
  ...
  const Nuevo = () => {
    navegacion(`/sistema/inventario/nuevamarca`);
  };
