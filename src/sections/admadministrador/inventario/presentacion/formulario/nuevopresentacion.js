...
export default function nuevamarca() {
  ...
  const validation = () => {
    ...
    if (nombre === '') {
      messajeTool('error', 'Debe asignar un nombre de la presentación.');
      setError(true);
      return false;
    }
  ...
  const Grabar = async () => {
    ...
    try {
      const { data } = await axios.post(`${URLAPIGENERAL}/presentaciones`, Json, {
        headers: {
  ...
  async function contarDatos() {
    const letra = await axios(`${URLAPIGENERAL}/inicales/listar?opcion=ADMPRESENTA`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
